...
courseId:any;
  constructor(private formBuilder: FormBuilder, private services: ServicesService,private router:Router,private http:HttpClient,private parms:ActivatedRoute) {
    ...
    const url = `https://localhost:7225/api/courses/getCourseDataForTeacher/${token}/${id}/${this.courseId.courseId}`;
...
this.selectedImage=this.courseData.pictures
  ...
  deleteCourse(){
    ...
    const url = `https://localhost:7225/api/courses/delete course/${token}/${userid}/${this.courseId.courseId}`;
    Swal.fire({
